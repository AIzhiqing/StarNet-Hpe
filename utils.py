...
def normalize_vector( v, use_gpu=True):
    ...
    if use_gpu:
        cuda = 'cuda:3'
        v_mag = torch.max(v_mag, torch.autograd.Variable(torch.FloatTensor([1e-8]).to(cuda)))
    else:
...
def compute_euler_angles_from_rotation_matrices(rotation_matrices, full_range=False, use_gpu=True):
    full_range = True   #!
    ...
    #         sy2[i] = -sy[i]
    if full_range:
        sy2[R[:,0,0]<0] = -1*sy2[R[:,0,0]<0]

    # R[abs(R)<0.01] = 0
    # print("sy before", sy)
    x=torch.atan2(R[:,2,1], R[:,2,2])
    y=torch.atan2(-R[:,2,0], sy2)
    z=torch.atan2(R[:,1,0],R[:,0,0])
    # print("x, y, z:", x, y, z)
    # import pdb;pdb.set_trace()
    xs=torch.atan2(-R[:,1,2], R[:,1,1])
    ys=torch.atan2(-R[:,2,0], sy2)
    zs=R[:,1,0]*0
    ...
    yaw[yaw > np.pi] = -2*np.pi + yaw[yaw > np.pi]
    yaw[yaw < -np.pi] = 2*np.pi + yaw[yaw < -np.pi]
    pitch[pitch > 0.5*np.pi] = -np.pi + pitch[pitch > 0.5*np.pi]  # [-90,90]
    pitch[pitch < -0.5*np.pi] = np.pi + pitch[pitch < -0.5*np.pi]
    roll[roll > 0.5*np.pi] = -np.pi + roll[roll > 0.5*np.pi]  # [-90,90]
    roll[roll < -0.5*np.pi] = np.pi + roll[roll < -0.5*np.pi]
    out_euler[:, 0] = pitch
    out_euler[:, 1] = yaw
    out_euler[:, 2] = roll
